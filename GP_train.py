...
class GP:
    ...
    def likelihood_gradients(self):
        ...
        A = np.matmul(
            Kinv, np.matmul(self.Y_s, np.matmul(omegainv, np.transpose(self.Y_s)))
        )
        dL_dl = (d / 2) * np.trace(np.matmul(Kinv, Kl)) + (1 / 2) * np.trace(
            np.matmul(-Kinv, np.matmul(Kl, A))
        )
        dL_ds = (d / 2) * np.trace(np.matmul(Kinv, Ks)) + (1 / 2) * np.trace(
            np.matmul(-Kinv, np.matmul(Ks, A))
        )
        dL_domega = (n / 2) * np.transpose(omegainv) - (1 / 2) * np.matmul(
            np.matmul(
                np.matmul(
                    np.matmul(np.transpose(omegainv), np.transpose(self.Y_s)),
                    np.transpose(Kinv),
                ),
                self.Y_s,
            ),
            np.transpose(omegainv),
        )
    ...
    def log_likelihood(self):
        ...
        A = np.matmul(
            np.matmul(
                np.matmul(np.linalg.inv(self.K), self.Y_s), np.linalg.inv(self.omega)
            ),
            np.transpose(self.Y_s),
        )
        L = (
            (n * d / 2) * np.log(2 * np.pi)
            + (d / 2) * np.log(np.linalg.det(self.K))
            + (n / 2) * np.log(np.linalg.det(self.omega))
            + (1 / 2) * np.trace(A)
        )
        return L
...
if __name__ == "__main__":
    ...
    for iteration in range(5, 10):
        ...
        gp = GP(
            ...
            omega=omega_init,
            L=L_init,
            l=10.0 + 70.0 * np.random.rand(),
            sigma=8.0 + 16.0 * np.random.rand(),
            noise=0.001,
        )
        ...
        while iters < max_iters:
            ...
            if (
                np.amax(dL_domega) > omega_grad_max
                or np.amin(dL_domega) < omega_grad_max
            ):
            ...
            if iters % 50 == 0 and kSave:
                ...
                np.save(
                    "parameters_human_v" + str(iteration),
                    [params_omega, params_sigma, params_l],
                )
        ...
        gp = GP(
            ...
            Y_r,
            omega=omega_init,
            l=0.2 + 3.0 * np.random.rand(),
            sigma=0.01 + 0.2 * np.random.rand(),
            noise=0.001,
        )
        ...
        while iters < max_iters:
            ...
            if iters % 50 == 0 and kSave:
                ...
                np.save(
                    "parameters_robot_v" + str(iteration),
                    [params_omega, params_sigma, params_l],
                )
